...
const Questions = ({ question, activeIndex, setActiveIndex }) => {
        ...
        <div className='p-5 border rounded-lg my-10'>
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2'>
                {question.map((item, index) => (
                    <div
                        key={index}
                        className={`p-2 border rounded-full text-xs md:text-sm text-center cursor-pointer ${activeIndex === index ? 'bg-primary text-white' : 'bg-secondary text-gray-700'} hover:bg-primary hover:text-white`}
